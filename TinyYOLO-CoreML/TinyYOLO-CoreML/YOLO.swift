...
//  let model = TinyYOLO()
  ...
  public func predict(image: CVPixelBuffer) -> [Prediction]? {
    if let output = try? model.prediction(image: image) {
      ...
      let list = [output._727, output._740]
      var pred = [Prediction]()
      for i in list {
          let cbb = computeBoundingBoxes(features: i) //return perdictions
          for z in 0..<cbb.count {
              pred.append(cbb[z])
//          print(pred)
          }
      }
        
//      return computeBoundingBoxes(features: output._714) //714, 727, 740
...
//          let tc = featurePointer[(channel * count) + 4]
          ...
          var z = b
          if features.count == 85 * 40 * 40 * 3 {
                z += 3
          }
          let w = exp(tw) * anchors[2*z    ] * blockSize
          let h = exp(th) * anchors[2*z + 1] * blockSize
...
//            if detectedClass == 7 , confidenceInClass > 0.7 {
